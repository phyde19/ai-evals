...
## Ways to use guidelines judgesâ

MLflow provides the following guidelines judges:

  1. **Built-in`Guidelines()` judge**: Apply global guidelines uniformly to all rows. Evaluates app inputs and outputs only. Works in both offline evaluation and production monitoring.
  2. **Built-in`ExpectationsGuidelines()` judge**: Apply per-row guidelines labeled by domain experts in an evaluation dataset. Evaluates app inputs and outputs only. For offline evaluation only.
...
## How guidelines workâ

Guidelines judges use a specially-tuned LLM to evaluate whether text meets your specified criteria. The judge:

  1. **Receives context** : Any JSON dictionary containing the data to evaluate (for example, `request`, `response`, `r`etrieved_documents`, `user_preferences`). You can reference these keys by name directly in your guidelines. See Reference context variables
...
For more information about the models powering the LLM judges, see Information about the models powering LLM judges.
...
##  `Guidelines()` judge: global guidelinesâ
...
Parameter| Type| Required| Description| `name`| `str`| Yes| Name for the judge, displayed in evaluation results| `guidelines`| `str | list[str]`| Yes| Guidelines to apply uniformly to all rows| `model`| `str`| No| Custom judge model  
---|---|---|---  
  
### How the Guidelines judge parses your app's inputs and outputsâ
...
##  `ExpectationsGuidelines()` judge: per-row guidelinesâ
...
## Best practices for writing effective guidelinesâ
...
**Be specific and measurable** â "The response must not include specific pricing amounts or percentages" â "Don't talk about money"

**Use clear pass/fail conditions** â "If asked about pricing, the response must direct users to the pricing page" â "Handle pricing questions appropriately"

**Reference context explicitly** â "The response must only use facts present in retrieved_context" â "Be factual"
...
#### Per-row guidelines for specific scenariosâ
...
#### Per-row guidelines for document typesâ
...
## Next stepsâ
  ...
  * [Use built-in LLM judges](</aws/en/mlflow3/genai/eval-monitor/concepts/scorers>) \- Evaluate quality with MLflow's other research-backed, built-in LLM judges
  * [Create custom LLM judges](</aws/en/mlflow3/genai/eval-monitor/custom-judge/>) \- Build custom judges for your specific needs
  * [Align judges with human feedback](</aws/en/mlflow3/genai/eval-monitor/align-judges>) \- Improve judge accuracy to match your quality standards