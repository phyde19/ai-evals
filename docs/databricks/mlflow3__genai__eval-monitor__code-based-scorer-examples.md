...
In MLflow Evaluation for GenAI, [custom code-based scorers](</aws/en/mlflow3/genai/eval-monitor/custom-scorers>) allow you to define flexible evaluation metrics for your AI agent or application. This set of examples and the companion example notebook illustrate many patterns for using code-based scorers with different options for inputs, outputs, implementation, and error handling.
...
### Define your GenAI appâ

Some examples below will use the following GenAI app, which is a general assistant for question-answering. The code below uses an OpenAI client to connect to [Databricks-hosted LLMs](</aws/en/machine-learning/foundation-model-apis/supported-models>).
...
## Example 1: Access data from the `Trace`â

Access the full [MLflow Trace object](</aws/en/mlflow3/genai/tracing/tracing-101>) to use various details (spans, inputs, outputs, attributes, timing) for fine-grained metric calculation.
...
## Example 2: Wrap a predefined LLM judgeâ

Create a custom scorer that wraps MLflow's [built-in LLM judges](</aws/en/mlflow3/genai/eval-monitor/concepts/scorers>). Use this to preprocess trace data for the judge or post-process its feedback.
...
### Example 3.1: Exact match with expected responseâ
...
### Example 3.2: Keyword check from expectationsâ
...
## Example 4: Return multiple feedback objectsâ
...
Each `Feedback` object should have a unique `name`, which becomes the metric name in the results. See the [details on metric names](</aws/en/mlflow3/genai/eval-monitor/custom-scorers#metric-naming>).
...
## Example 5: Use your own LLM for a judgeâ
...
## Example 6: Class-based scorer definitionâ
...
## Example 7: Error handling in scorersâ
...
##  Example 8: Naming conventions in scorersâ

The following examples illustrate the [naming behavior for code-based scorers](</aws/en/mlflow3/genai/eval-monitor/custom-scorers#metric-naming>). The behavior can be summarized as:
...
## Example 9: Chaining evaluation resultsâ
...
## Example 10: Conditional logic with guidelinesâ

You can wrap [Guidelines judges](</aws/en/mlflow3/genai/eval-monitor/concepts/judges/guidelines>) in custom code-based scorers to apply different guidelines based on user attributes or other context.
...
[Open notebook in new tab](<https://docs.databricks.com/aws/en/notebooks/source/mlflow3/code-based-scorer-examples.html>)
